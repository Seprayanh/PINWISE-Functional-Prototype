...
export function PlanScreen({destination,view,setView,places,itinerary,setItinerary,onPlace,onDecision,onReoptimize,onConflict,finalized,onFinalize}:{
  ...
  function remove(dayId:number,itemId:string,place:Place){
    setItinerary(all=>all.map(d=>d.id===dayId?{...d,items:d.items.filter(i=>i.id!==itemId)}:d));onDecision(place,'removed')
  }
  function addPlace(place:Place){
    const first=itinerary[0];if(!first||first.items.some(i=>i.placeId===place.id))return
    setItinerary(all=>all.map(d=>d.id===first.id?{...d,items:[...d.items,{id:`item-${Date.now()}`,time:'16:00',placeId:place.id}]}:d));onDecision(place,'kept')
  }
  ...
  return <div className="screen-scroll">
    ...
    <main className="screen-content plan-content">
      ...
      {view==='map'&&<div className="map-panel">
        ...
        <div className="map-legend"><span><i className="dot current"/>Current</span><span><i className="dot review"/>Review</span><span><i className="dot outdated"/>Outdated</span><span><i className="dot unverified"/>Candidate</span></div>
        <div className="map-summary"><div><strong>{places.length}</strong><span>places</span></div><div><strong>{selected.length}</strong><span>selected</span></div><div><strong>{places.filter(p=>p.selected&&['review','outdated','unverified'].includes(p.status)).length}</strong><span>needs review</span></div></div>
      </div>}
