...
function pinIcon(place:Place,focused:boolean){
  ...
  return divIcon({
    className:'',
    html:`<div style="width:${size}px;height:${size}px;border-radius:50% 50% 50% 10px;transform:rotate(-45deg);background:${color};border:3px solid #fff;box-shadow:0 5px 16px rgba(46,41,16,.26);display:grid;place-items:center;${selectedRing}${focusRing}"><span style="transform:rotate(45deg);color:#fff;font-size:11px;font-weight:900;font-family:-apple-system,BlinkMacSystemFont,'SF Pro Text','Helvetica Neue',sans-serif;line-height:1;letter-spacing:-.02em">${initials}</span></div>`,
...
function SyncBounds({places}:{places:Place[]}){
  const map=useMap(),prevCount=useRef(places.length)
  useEffect(()=>{
    ...
    const lats=pts.map(p=>p.lat),lngs=pts.map(p=>p.lng)
    map.fitBounds([[Math.min(...lats),Math.min(...lngs)],[Math.max(...lats),Math.max(...lngs)]],{padding:[48,48],maxZoom:14,animate:true})
  },[map,places]);return null
}
